...
function CalendarMonth({placeholder, onSetValue, value, icon, disableReset}: any) {
	...
	return (
		<HeadlessTippy
			...
			placement='bottom'
			render={(attrs: any) => (
				<MainCalendar onSetValue={onSetValue} value={value} onClose={() => setShow(false)} disableReset={disableReset} />
			)}
...
const MainCalendar = ({
	...
	const [month, setMonth] = useState<number | null>(value ? value.getMonth() + 1 : value);
